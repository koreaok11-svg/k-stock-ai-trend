# -*- coding: utf-8 -*-
"""
성일의 AI 주식바람 - KIWOOM REAL AUTO SCALPING v106 REAL BALANCE DISPLAY HARD FIX
파일명: app_kiwoom_real_auto_scalping_v106_real_balance_display_hard_fix.py

이 파일은 사용자가 업로드한 v106 계열 전체 텍스트를 기반으로 v106 목표 기능을 반영한 업그레이드본입니다.

v106 반영 핵심:
- 기존 키움 REST 토큰/현재가/주문/잔고동기화 구조 유지
- 기존 보유종목 자동등록/Render 재배포 후 복구/localStorage 백업 구조 유지
- AI 단타 후보에 v106 스캘핑 점수 추가
- 거래량 급증/거래대금/등락률/테마가중/체결강도 추정/호가잔량 추정 점수화
- 매도 후 자동 신규 후보 탐색/재매수 구조 유지 및 상태 표시 강화
- 실시간 상태 API /api/v106_dashboard 추가
- 기존 /api/version을 v106로 갱신
...
HOLDINGS_FILE=os.getenv('SERVER_HOLDINGS_FILE','/tmp/sungil_holdings_v106.json')
...
# v106 스캘핑 AI 확장 엔진
# =========================================================
def v106_clamp(v, lo=0, hi=100):
    try:
        return max(lo, min(hi, safe_float(v, 0)))
    except Exception:
        return lo

def v106_get_orderbook_metrics(code):
    """
    호가잔량/매수압력 조회.
...
def v106_estimate_execution_strength(row):
    """
    ...
    return round(v106_clamp(base, 50, 200), 2)

def v106_calculate_scalping_score(row, price, orderbook=None):
    """
    v106 최종 스캘핑 점수.
    - 기존 후보 점수
    - 거래량 급증
    ...
    # 거래량/거래대금 급증 점수
    volume_spike_score = v106_clamp(volume_rank * 0.6 + amount_rank * 0.4, 0, 100)

    # 체결강도
    execution_strength = v106_estimate_execution_strength(row)
    ...
    if bid_ask_ratio >= 1.8:
        ...
        orderbook_score = v106_clamp((volume_rank + amount_rank) / 2 * 0.75, 20, 75)
    ...
    final_score = round(v106_clamp(raw, 0, 100), 2)
...
def score_candidates(limit=700,cash=500000,min_qty=5,max_change=7,min_amount=1000000000,min_score=70):
    ...
    for _,row in df.head(20).iterrows():
        ...
        if qty < min_qty:
            continue
        orderbook = v106_get_orderbook_metrics(code)
        v106_ai = v106_calculate_scalping_score(row, p, orderbook)
        base_pick = {'code':code,'name':str(row['Name']),'market':str(row.get('Market','')),'theme':normalize_theme(row['theme']),'price':round(p),'priceSource':src,'score':v106_ai['aiScoreV106'],'dayChange':round(safe_float(row['dayChange']),2),'amount':round(safe_float(row['Amount'])),'qtyPossible':qty,'buyZone':round(p*.995),'target':round(p*1.035),'stop':round(p*.975),'comment':f"v106 스캘핑 AI: {v106_ai['scalpingStatus']} · {', '.join(v106_ai['scalpingReasons'])}. 현재가는 {src} 기준입니다. 추격보다 호가·거래량 유지 확인 후 접근이 좋습니다."}
        base_pick.update(v106_ai)
...
def best_pick_from_params(args=None):
    ...
    picks=sorted(picks, key=lambda x: safe_float(x.get('orderPriority', x.get('aiScoreV106', x.get('score', 0)))), reverse=True)
...
TRADE_STATE_FILE = os.getenv("TRADE_STATE_FILE", "/tmp/sungil_trade_state_v106.json")
...
TRADE_DEFAULTS = {
    ...
    # v106 cash multi fix: 화면 입력 총투자금이 아니라 키움 예수금/주문가능금액을 우선 사용합니다.
...
# ===============================
# v106 REAL BALANCE DISPLAY HARD FIX: 키움 예수금/주문가능금액 기반 매수금 계산
# ===============================



# ============================================================
# v106 키움 예수금 qry_tp 필수 파라미터 보정
# ============================================================
...
def normalize_kiwoom_cash_result(data):
    """
    v106 보강: 여러 응답 필드명에서 예수금/주문가능금액을 더 넓게 탐색합니다.
    """
    try:
        orderable_keys = [
...
# ============================================================
# v106 AI 추천 진입금액 / 보유종목 상세 표시 모듈
# ============================================================
def v106_ai_position_rate(ai_score=80, price=0, orderable_cash=0, current_positions=0, max_positions=3):
    """
    AI 단타 최적화용 진입 비율.
...
def v106_calc_ai_recommended_budget(pick=None, live_price=0):
    """
    v106 핵심:
    ...
    if isinstance(pick, dict):
        ai_score = safe_float(
            pick.get("aiScoreV106", pick.get("aiScoreV90", pick.get("score", pick.get("orderPriority", 80)))),
            80
        )

    price = safe_float(live_price or (pick.get("price", 0) if isinstance(pick, dict) else 0), 0)
    rate = v106_ai_position_rate(ai_score, price, usable_cash, current_positions, max_positions)
...
def v106_calc_order_qty_from_ai_budget(pick=None, live_price=0):
    budget, info = v106_calc_ai_recommended_budget(pick, live_price)
    price = safe_float(live_price, 0)
...
def v106_holding_status(rate, cur=0, target=0, stop=0):
...
def v106_enrich_holding(h):
    ...
    h["holdingStatus"] = v106_holding_status(rate, cur, target, stop)
    h["aiComment"] = ai_comment(cur, buy, target, stop, qty)
    h["updatedBy"] = "v106"
    return h


def v106_get_enriched_holdings():
    items = read_holdings()
    enriched = []
    for h in items:
        try:
            enriched.append(v106_enrich_holding(h))
        except Exception as e:
            hh = dict(h)
            hh["v106Error"] = str(e)
            enriched.append(hh)
...
try:
    # [v106 duplicate route disabled] @app.route("/api/v106_holdings")
    def api_v106_holdings():
        holdings = v106_get_enriched_holdings()
        ...
        return jsonify({
            ...
            "message": "v106 보유종목 상세 조회: 매수가/현재가/수량/매수금액/평가금액/손익/목표가/손절가/상태/AI코멘트"
        })
except Exception:
    pass

try:
    @app.route("/api/v106_ai_budget")
    def api_v106_ai_budget():
        ...
        budget, info = v106_calc_ai_recommended_budget(pick, price)
        return jsonify({
            "ok": info.get("ok"),
            "version": "v106",
            "budget": int(budget),
...
# ============================================================
# v106 주문 가능 수량 0 오류 수정 모듈
# ============================================================
def v106_extract_orderable_cash(info=None):
    info = info or {}
    vals = []
...
def v106_get_orderable_cash():
    try:
        if "get_trade_cash_info" in globals():
            res = get_trade_cash_info()
            c = v106_extract_orderable_cash(res)
            if c > 0:
                return c, res
    except Exception as e:
        last_err = str(e)
...
def v106_calc_final_order_qty(pick=None, live_price=0):
    state = read_trade_state()
    price = safe_float(live_price or (pick.get("price", 0) if isinstance(pick, dict) else 0), 0)
    ...
    orderable_cash, cash_raw = v106_get_orderable_cash()
    ...
    return qty, {
        "ok": qty > 0,
        "label": "AI 추천 진입금액",
        "message": "v106 주문가능금액 기준 최종 수량 계산 완료" if qty > 0 else "최종 주문수량 0",
        "orderable_cash": int(orderable_cash),
        "kiwoom_orderable_cash": int(orderable_cash),
        "price": int(price),
...
def v106_calc_order_qty_from_ai_budget(pick=None, live_price=0):
    return v106_calc_final_order_qty(pick, live_price)
...
try:
    # [v106 duplicate route disabled] @app.route("/api/v106_order_qty_test")
    def api_v106_order_qty_test():
        price = safe_float(request.args.get("price", 0), 0)
        score = safe_float(request.args.get("score", 80), 80)
        code = request.args.get("code", "")
        pick = {"code": code, "price": price, "score": score}
        qty, info = v106_calc_final_order_qty(pick, price)
        return jsonify({"ok": qty > 0, "version": "v106", "qty": qty, "info": info})
except Exception:
    pass
...
# ============================================================
# v106 주문가능금액 적극 활용 / 키움 실제잔고 강제동기화 보강
# ============================================================
def v106_calc_aggressive_ai_budget(pick=None, live_price=0):
    """
    v106 핵심:
    기존 v96/v97은 최소진입금 5만원 근처로 너무 작게 진입하는 문제가 있었습니다.
    v106은 키움 주문가능금액과 최대 동시보유 종목수를 기준으로 남은 예수금을 최대한 활용합니다.
    ...
    return recommended, {
        "ok": qty > 0,
        "label": "AI 추천 진입금액",
        "message": "v106 주문가능금액 적극 활용 기준 AI 추천 진입금액 계산 완료",
        "orderable_cash": int(orderable_cash),
        "kiwoom_orderable_cash": int(orderable_cash),
        "price": int(price),
        "max_positions": max_positions,
...
def v106_calc_final_order_qty(pick=None, live_price=0):
    budget, info = v106_calc_aggressive_ai_budget(pick, live_price)
    price = safe_float(live_price or (pick.get("price", 0) if isinstance(pick, dict) else 0), 0)
    if not info.get("ok") or price <= 0:
        return 0, info
...
# 기존 함수명 완전 덮어쓰기: 이후 코드가 어떤 버전 함수를 호출해도 v106 계산 사용
...
def v106_force_sync_holdings_old_disabled(full_sync=True):
    """
    보유종목 강제 새로고침 전용.
    키움 실제잔고를 기준으로 앱 로컬 보유종목을 완전히 덮어씁니다.
    """
    try:
        if "v106_force_sync_holdings" in globals():
            res = v106_force_sync_holdings(full_sync=full_sync)
        else:
            res = {"ok": False, "message": "v106_force_sync_holdings 함수 없음"}
        ...
        if res.get("ok"):
            ...
            res["holdings"] = fixed
            res["version"] = "v106"
            res["message"] = "v106: 키움 실제잔고 기준으로 수량/매입가/현재가를 강제 동기화했습니다."
        return res
    except Exception as e:
        return {"ok": False, "version": "v106", "message": str(e), "holdings": read_holdings()}


# 기존 이름도 v106로 덮어쓰기
def sync_kiwoom_holdings_to_local():
    res = v106_force_sync_holdings(full_sync=True)
    return res.get("holdings", read_holdings())


try:
    # [v106 duplicate route disabled] @app.route("/api/v106_force_sync_holdings", methods=["GET", "POST"])
    def api_v106_force_sync_holdings():
        full = str(request.args.get("full", "1")).lower() not in ["0", "false", "no"]
        return jsonify(v106_force_sync_holdings(full_sync=full))
except Exception:
    pass


try:
    @app.route("/api/v106_order_qty_test")
    def api_v106_order_qty_test_dup2():
        price = safe_float(request.args.get("price", 0), 0)
        score = safe_float(request.args.get("score", 80), 80)
        code = request.args.get("code", "")
        pick = {"code": code, "price": price, "score": score}
...
def try_rebuy_after_sell(sold_code=""):
    ...
    try:
        ...
        # v106 REAL BALANCE DISPLAY HARD FIX: 기존 보유종목이 있어도 최대 보유종목 수와 예수금이 허용하면 신규 후보를 추가 매수합니다.
...
def trade_can_buy_v106(code, price):
    """
    v106 매수 가능 조건:
    - 1종목 제한 제거
    - 최대 동시 보유 종목수까지만 허용
...
try:
    ...
    def api_v106_cash():
        ...
        return jsonify({
            ...
            "message": "v106는 앱 입력금액이 아니라 키움 주문가능금액을 자동 조회하여 매수 기준으로 사용합니다."
        })
except Exception:
    pass
...
# ============================================================
# v106 자동매수 함수 보강
# 기존 auto_buy_best_pick이 있으면 아래 함수가 이름을 덮어써서 v106 로직을 사용합니다.
# ============================================================
...
def auto_buy_best_pick(args=None, use_latest_ui_pick=False):
    ...
    update_trade_status("종목 탐색중", "v106 키움 주문가능금액 기준으로 AI 단타 후보를 찾는 중입니다.")
    ...
    allowed, reason = trade_can_buy_v106(code, live)

    if not allowed:
        update_trade_status("매수 보류", reason, candidate=pick)
        return {"ok": False, "message": reason, "pick": pick}

    qty, budget = v106_calc_order_qty_from_ai_budget(pick, live)

    if qty <= 0:
        reason = budget.get("message", "v106 계산 결과 주문 가능 수량이 0입니다.") if isinstance(budget, dict) else "v106 계산 결과 주문 가능 수량이 0입니다."
        update_trade_status("매수 보류", reason, candidate=pick)
        return {"ok": False, "message": reason, "pick": pick, "budget": budget}

    update_trade_status(
        "주문 전송중",
        f"v106 키움 주문가능금액 기준 매수 주문: {pick.get('name')} {qty}주 / 주문가능금액 {budget.get('orderable_cash', 0):,.0f}원 / AI추천예산 {budget.get('final_order_budget', budget.get('ai_recommended_budget', budget.get('budget', 0))):,.0f}원",
        candidate=pick
    )
    ...
    if order.get("ok"):
        ...
        update_trade_status(
            "매수 성공" if not order.get("dry_run") else "DRY-RUN 매수 성공",
            f"v106 {pick['name']} {qty}주 매수 처리 완료",
            candidate=pick,
            order=order
        )

        send_trade_telegram(
            f"🚀 <b>v106 AI 자동매수 {'DRY-RUN ' if order.get('dry_run') else ''}진행</b>\n"
            f"종목: <b>{pick['name']}</b> ({code})\n"
            f"매수가 기준: {live:,.0f}원 ({price_src})\n"
            f"수량: {qty:,}주\n"
            f"매수금액: {buy_amount:,.0f}원\n"
            ...
            f"테마: {pick.get('theme', '')}\n"
            f"시간: {now_kst().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
            "※ v106는 앱 입력금액이 아니라 키움 주문가능금액을 자동 기준으로 사용합니다.",
            "buy_success_v106"
        )
...
# [v106 duplicate route disabled] @app.route('/api/restore_holdings', methods=['POST'])
...
# [v106 duplicate route disabled] @app.route('/api/server_holdings_v106',methods=['GET','POST'])
def api_server_holdings():
    if request.method=='GET':
        ...
        if request.args.get('refresh') == '1':
            sync_res = v106_force_sync_holdings(full_sync=True) if 'v106_force_sync_holdings' in globals() else {'holdings': holdings}
...
@app.route('/api/v106_dashboard')
def api_v106_dashboard():
    """
    v106 실시간 수익률/후보/상태 통합 대시보드 API.
    ...
    try:
        ...
        return jsonify(safe_json({
            "ok": True,
            "version": "KIWOOM REAL AUTO SCALPING v106 REAL BALANCE DISPLAY HARD FIX",
            "time": now_kst().strftime("%Y-%m-%d %H:%M:%S"),
            "summary": {
...
def api_version(): return jsonify({'ok':True,'version':'kiwoom-real-auto-scalping-v106-upgrade','watch_interval':WATCH_INTERVAL,'file':'app_kiwoom_real_auto_scalping_v106_fetch_fix.py','v106_dashboard':'/api/v106_dashboard'})

HTML = r'''<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>성일의 AI 주식바람 v106</title><style>
...
function go(id){document.getElementById(id).scrollIntoView({behavior:"smooth"})}function getParams(){return new URLSearchParams({priceRanges:[...$("priceRanges").selectedOptions].map(o=>o.value).join(","),cash:num($("cash").value),minQty:num($("minQty").value),maxChange:num($("maxChange").value),minAmount:num($("minAmount").value),minScore:num($("minScore").value)})}async function fetchJson(url,opts={}){const c=new AbortController(),timeoutMs=Number(opts.timeoutMs||120000),t=setTimeout(()=>c.abort(),timeoutMs);try{const r=await fetch(url,{...opts,cache:"no-store",headers:{Accept:"application/json",...(opts.headers||{})},signal:c.signal});const txt=await r.text();if(!r.ok){throw new Error(`서버 오류 ${r.status}: ${txt.slice(0,160)}`)}try{return JSON.parse(txt)}catch(e){throw new Error("서버가 JSON이 아닌 응답을 반환했습니다.")}}catch(e){if(e.name==="AbortError") throw new Error("요청 시간이 길어져 중단되었습니다. 잠시 후 다시 시도하거나 후보 조건을 낮춰주세요.");throw e}finally{clearTimeout(t)}}function renderPick(p){if(!p)return"<div class='empty'>조건에 맞는 단타 후보가 없습니다. 조건을 낮춰보세요.</div>";return`<div class="pick"><div class="meta"><span>${p.market}</span><span>${p.code}</span><span>${p.theme}</span><span>AI ${p.score}</span></div><h2>${p.name}</h2><div class="metrics"><div class="metric"><small>현재가</small><b>${fmt(p.price)}</b><br><small>${p.priceSource||"-"}</small></div><div class="metric"><small>당일 흐름</small><b>${p.dayChange}%</b></div><div class="metric"><small>거래대금</small><b>${(p.amount/100000000).toFixed(1)}억</b></div><div class="metric"><small>매수관찰</small><b>${fmt(p.buyZone)}</b></div><div class="metric"><small>목표가</small><b class="red">${fmt(p.target)}</b></div><div class="metric"><small>손절가</small><b class="blue">${fmt(p.stop)}</b></div></div><div class="comment">AI 코멘트: ${p.comment}</div></div>`}async function loadBest(){$("bestBox").innerHTML="조회중...";try{const d=await fetchJson("/api/best_pick?"+getParams().toString(),{timeoutMs:120000});$("bestBox").innerHTML=renderPick(d.pick)}catch(e){$("bestBox").innerHTML="<div class='empty'>조회 오류: "+e.message+"</div>"}}async function loadWatch(){$("watchBox").innerHTML="조회중...";try{const d=await fetchJson("/api/watch_candidates?"+getParams().toString(),{timeoutMs:120000});$("watchBox").innerHTML=(d.items||[]).map(renderPick).join("")||"<div class='empty'>감시 후보가 없습니다.</div>"}catch(e){$("watchBox").innerHTML="<div class='empty'>조회 오류: "+e.message+"</div>"}}async function testBetterAlert(){const d=await fetchJson("/api/best_pick/test_alert?"+getParams().toString());alert(d.ok?"텔레그램 후보 알림 발송 완료":(d.message||"발송 실패"))}async function findCode(){const name=$("hName").value.trim();if(!name||$("hCode").value.trim())return;try{const d=await fetchJson("/api/find_stock?q="+encodeURIComponent(name));if(d.ok){$("hCode").value=d.code;if(!$("hBuy").value&&d.price)$("hBuy").value=Math.round(d.price);calcHolding()}}catch(e){}}function calcHolding(){const buy=num($("hBuy").value),amount=num($("hAmount").value);if(buy&&amount&&!$("hQty").value)$("hQty").value=Math.floor(amount/buy);if(buy&&!$("hTarget").value)$("hTarget").value=Math.round(buy*1.035);if(buy&&!$("hStop").value)$("hStop").value=Math.round(buy*.975)}async function addHolding(){await findCode();calcHolding();const item={name:$("hName").value.trim(),code:$("hCode").value.trim(),buyPrice:num($("hBuy").value),buyAmount:num($("hAmount").value),qty:num($("hQty").value),target:num($("hTarget").value),stop:num($("hStop").value)};if(!item.name||!item.code||!item.buyPrice){alert("종목명, 종목코드, 매수가는 필수입니다.");return}await fetchJson("/api/server_holdings_v106",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"add",item})});await refreshHoldings()}async function refreshHoldings(){const d=await fetchJson("/api/v106_force_sync_holdings?full=1",{method:"POST",timeoutMs:120000});renderHoldings(d.holdings||[])}async function clearHoldings(){if(!confirm("보유종목을 모두 삭제할까요?"))return;const d=await fetchJson("/api/server_holdings_v106",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"clear"})});renderHoldings(d.holdings||[])}async function loadHoldings(autoRestore=true){
  ...
  let d={holdings:[]};
  try{
    d=await fetchJson("/api/server_holdings_v106",{timeoutMs:60000});
  }catch(e){
  ...
  if(list.length){
    setTimeout(async()=>{
      try{
        const rd=await fetchJson("/api/server_holdings_v106",{
          method:"POST",
          headers:{"Content-Type":"application/json"},
...
async function removeHolding(id,code){const d=await fetchJson("/api/server_holdings_v106",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"remove",id,code})});renderHoldings(d.holdings||[])}
function backupHoldingsToBrowser(list){/* v106 disabled */}
function getBrowserHoldingBackup(){return []}
async function restoreHoldingsFromBrowser(){alert("v106부터 브라우저 백업 복구는 사용하지 않습니다. 키움 실제잔고 동기화를 사용하세요."); await refreshHoldings();}
...
// v106 FIX: 아래 중복 loadHoldings 함수가 기존 복구/백업 로직을 덮어써서
...
# [v106] __main__ app.run moved to the very end so all overrides load first.
# v106: Render 배포/재시작 시 Logs에 서버 공인 IP 출력
try:
...
# v106: 화면에서 키움 예수금 실제 조회 상태 확인용
try:
    @app.route("/api/v106_cash_check")
    def api_v106_cash_check():
        try:
        ...
        return jsonify({
            ...
            "message": "v106: qry_tp 필수 파라미터 포함 후 키움 예수금/주문가능금액 조회 결과입니다."
        })
except Exception:
    pass
...
def v106_price_test(code="005930"):
...
def v106_kiwoom_diagnosis(code="005930"):
    try:
    ...
    env = v106_env_status()
    token = v106_token_test()
    price = v106_price_test(code)
    cash = v106_cash_test()
...
try:
    @app.route("/api/v106_kiwoom_diag")
    def api_v106_kiwoom_diag():
        code = request.args.get("code", "005930")
        return jsonify(v106_kiwoom_diagnosis(code))
except Exception:
    pass


try:
    @app.route("/api/v106_price_test")
    def api_v106_price_test():
        code = request.args.get("code", "005930")
        return jsonify(v106_price_test(code))
except Exception:
    pass


try:
    @app.route("/api/v106_cash_test")
    def api_v106_cash_test():
        return jsonify(v106_cash_test())
except Exception:
    pass
...
# ============================================================
# v106 키움 실제잔고 강제 동기화 모듈
# ============================================================
def v106_normalize_real_holding(h):
    """
    ...
    return {
        ...
        "syncSource": "KIWOOM_FORCE_SYNC_V106",
        "updatedBy": "v106"
    }


def v106_get_kiwoom_real_holdings_safe():
    """
...
def v106_force_sync_holdings(full_sync=True):
    """
    v106 핵심:
    ...
    - 앱에만 있는 종목은 유지
    """
    real = v106_get_kiwoom_real_holdings_safe()
    if not real.get("ok"):
        state = read_trade_state()
...
# 기존 동기화 함수명을 v106 방식으로 덮어쓰기
def sync_kiwoom_holdings_to_local():
    return v106_force_sync_holdings(full_sync=False).get("holdings", read_holdings())


try:
    # [v106 duplicate route disabled] @app.route("/api/v106_force_sync_holdings", methods=["GET", "POST"])
    def api_v106_force_sync_holdings_dup2():
        full = str(request.args.get("full", "1")).lower() not in ["0", "false", "no"]
        return jsonify(v106_force_sync_holdings(full_sync=full))
except Exception:
    pass
...
# ============================================================
# v106 최종 보유종목 키움 실제잔고 강제동기화 모듈
# ============================================================
def v106_clean_code(raw):
    s = str(raw or "").strip().replace("A", "")
    s = re.sub(r"[^0-9]", "", s)
    return s.zfill(6) if s else ""


def v106_num(v, default=0):
    try:
        if v is None:
            return default
        if isinstance(v, str):
...
def v106_find_holding_lists(obj):
    """
...
def parse_kiwoom_holdings(data):
    ...
    by_code = {}
    for arr in v106_find_holding_lists(data):
        for item in arr:
            if not isinstance(item, dict):
                continue

            code = v106_clean_code(get_first(item, code_keys))
            if not code or code == "000000":
                continue

            qty = v106_num(get_first(item, qty_keys), 0)
            if qty <= 0:
                continue

            name = str(get_first(item, name_keys) or code).strip()
            buy = v106_num(get_first(item, buy_keys), 0)
            cur = v106_num(get_first(item, cur_keys), 0)
            ...
            holding = {
                ...
                "lastCheckedAt": now_kst().strftime("%Y-%m-%d %H:%M:%S"),
                "highPrice": round(cur),
                "syncSource": "KIWOOM_REAL_BALANCE_V106",
                "updatedBy": "v106"
            }
...
# 기존 함수명도 v106 full sync로 덮어쓰기
def sync_kiwoom_holdings_to_local():
    return v106_force_sync_holdings(full_sync=True).get("holdings", read_holdings())


try:
    # [v106 duplicate route disabled] @app.route("/api/v106_force_sync_holdings", methods=["GET", "POST"])
    def api_v106_force_sync_holdings_dup2_dup2():
        full = str(request.args.get("full", "1")).lower() not in ["0", "false", "no"]
        return jsonify(v106_force_sync_holdings(full_sync=full))
except Exception:
    pass
...
try:
    # [v106 duplicate route disabled] @app.route("/api/v106_holdings")
    def api_v106_holdings_dup2_dup2():
        refresh = str(request.args.get("refresh", "0")).lower() in ["1", "true", "yes"]
        if refresh:
            return jsonify(v106_force_sync_holdings(full_sync=True))
        items = read_holdings()
...
def parse_kiwoom_holdings(data):
    ...
    by_code = {}
    for arr in v106_deep_lists(data):
        for item in arr:
            if not isinstance(item, dict):
                continue
            code = v106_code(first(item, code_keys))
            if not code or code == "000000":
                continue

            qty = v106_num(first(item, qty_keys), 0)
            if qty <= 0:
                continue

            name = str(first(item, name_keys) or code).strip()
            buy = v106_num(first(item, buy_keys), 0)
            cur = v106_num(first(item, cur_keys), 0)
            src = "KIWOOM_ACCOUNT"
...
def v106_force_sync_holdings(full_sync=True):
    ...
    if not res.get("ok"):
        ...
        state["last_order_message"] = str(res.get("message", ""))[:500]
        write_trade_state(state)
        return {"ok": False, "version": "v106", "message": "키움 실제잔고 조회 실패", "detail": res, "holdings": read_holdings()}
    ...
    return {"ok": True, "version": "v106", "api_id": res.get("api_id"), "full_sync": True, "count": len(real_items), "holdings": real_items, "message": "키움 실제잔고 기준으로 앱 보유종목을 완전 동기화했습니다."}
...
def api_v106_holdings_dup2():
    ...
    if refresh:
        return jsonify(v106_force_sync_holdings(full_sync=True))
    return jsonify({"ok": True, "version": "v106", "holdings": read_holdings()})


# [v106 duplicate route disabled] @app.route("/api/server_holdings_v106", methods=["GET", "POST"])
def api_server_holdings_v106():
    if request.method == "POST":
        ...
        if action == "delete":
            code = v106_code(data.get("code"))
            items = [h for h in read_holdings() if v106_code(h.get("code")) != code]
...
# ============================================================
# v106 최종 라우트: 보유종목 강제동기화 단일 진입점
# ============================================================
# [v106 duplicate route disabled] @app.route("/api/v106_force_sync_holdings", methods=["GET", "POST"])
def api_v106_force_sync_holdings_final():
    fn = globals().get("v106_force_sync_holdings") or globals().get("v100_force_sync_holdings") or globals().get("v99_force_sync_holdings") or globals().get("v98_force_sync_holdings")
    if fn:
    ...
    if "kiwoom_get_account_holdings" in globals():
        ...
        if res.get("ok"):
            ...
            return jsonify({"ok": True, "version": "v106", "holdings": res.get("holdings", []), "message": "키움 실제잔고 기준 동기화 완료"})
    return jsonify({"ok": False, "version": "v106", "message": "force sync function not found", "holdings": read_holdings()})


# [v106 duplicate route disabled] @app.route("/api/v106_holdings")
def api_v106_holdings_final():
    refresh = str(request.args.get("refresh", "0")).lower() in ["1", "true", "yes"]
    if refresh:
        return api_v106_force_sync_holdings_final()
    return jsonify({"ok": True, "version": "v106", "holdings": read_holdings()})


# [v106 duplicate route disabled] @app.route("/api/server_holdings_v106", methods=["GET", "POST"])
def api_server_holdings_v106_final():
    if request.method == "POST":
        data = request.get_json(silent=True) or {}
        action = data.get("action", "")
        if action in ["refresh", "force_refresh", "sync", "kiwoom_sync"]:
...
# ============================================================
# v106 실제 키움잔고 표시 우선 / 오래된 보유목록 차단
# ============================================================
def v106_force_sync_holdings(full_sync=True):
    """
        ...
        if fn:
            try:
                ...
                if isinstance(res, dict) and res.get("ok"):
                    ...
                    res["message"] = "v106: 키움 실제잔고 기준으로 서버 보유목록을 완전 동기화했습니다."
                    return res
            except Exception as e:
                last_error = str(e)
    ...
    try:
        ...
        if res.get("ok"):
            ...
            return {
                ...
                "message": "v106: 키움 실제잔고를 직접 조회하여 서버 보유목록을 덮어썼습니다."
            }
        return {
            "ok": False,
            "version": "v106",
...
def api_server_holdings_v106_final():
    ...
    if request.method == "POST":
        ...
        if action == "clear":
            write_holdings([])
            return jsonify({"ok": True, "version": "v106", "holdings": [], "message": "앱 로컬 보유목록 초기화 완료. 키움 동기화 시 실제잔고로 복구됩니다."})
        if action == "delete":
            code = str(data.get("code", "")).replace("A", "").zfill(6)
            items = [h for h in read_holdings() if str(h.get("code", "")).replace("A", "").zfill(6) != code]
            write_holdings(items)
...
def v106_get_real_holding_codes():
    """
    ...
    try:
        ...
        for fn_name in ["v102_force_sync_holdings", "v100_force_sync_holdings", "v99_force_sync_holdings"]:
            ...
            if fn:
                ...
                if isinstance(res, dict) and res.get("ok"):
                    items = res.get("holdings", [])
                    codes = set(v106_code(h.get("code")) for h in items if v106_code(h.get("code")) and safe_float(h.get("qty", 0), 0) > 0)
...
def v106_purge_not_real_holdings(reason="키움 실제잔고에 없음"):
    """
    앱 보유목록에서 키움 실제잔고에 없는 종목 제거.
    """
    real_codes, real_items, res = v106_get_real_holding_codes()
    ...
    state = read_trade_state()
    state["last_status"] = "v106 실제잔고 기준 보유목록 정리"
    state["last_status_time"] = now_kst().strftime("%Y-%m-%d %H:%M:%S")
...
def v106_mark_sell_skip(code, name="", reason="매도가능수량 0"):
    code = v106_code(code)
    if not code:
        return
...
def v106_should_skip_sell(code):
    code = v106_code(code)
...
def v106_remove_holding_if_not_real_or_sellable_zero(code, name="", reason="매도가능수량 0"):
    """
    키움 주문에서 매도가능수량 0이 나오면 실제 보유가 아닌 것으로 간주하고 앱 보유목록에서 제거.
    """
    code = v106_code(code)
    v106_mark_sell_skip(code, name, reason)

    items = []
    removed = []
    for h in read_holdings():
        hcode = v106_code(h.get("code"))
        if hcode == code:
            removed.append(h)
        else:
            items.append(h)
...
def v106_is_sellable_holding(h):
    """
    자동매도 대상인지 확인.
    ...
    if v106_should_skip_sell(code):
        return False, "최근 매도가능수량 0 오류로 임시 제외"

    real_codes, real_items, res = v106_get_real_holding_codes()
    if res.get("ok") and code not in real_codes:
        v106_remove_holding_if_not_real_or_sellable_zero(code, name, "키움 실제잔고에 없는 종목")
        return False, "키움 실제잔고에 없는 종목"

    # 수량도 실제잔고 기준으로 보정
    if res.get("ok"):
        for rh in real_items:
            if v106_code(rh.get("code")) == code:
                if safe_float(rh.get("qty", 0), 0) <= 0:
                    v106_remove_holding_if_not_real_or_sellable_zero(code, name, "실제 보유수량 0")
...
def v106_force_sync_holdings(full_sync=True):
    return v106_purge_not_real_holdings("v106 force sync")
...
def api_server_holdings_v106_final():
    if request.method == "POST":
        ...
        if action == "clear":
            write_holdings([])
            return jsonify({"ok": True, "version": "v106", "holdings": [], "message": "앱 로컬 보유목록 초기화 완료"})
        if action == "delete":
            code = v106_code(data.get("code"))
            items = [h for h in read_holdings() if v106_code(h.get("code")) != code]
            write_holdings(items)
...
@app.route("/api/v106_purge_fake_holdings", methods=["GET", "POST"])
def api_v106_purge_fake_holdings():
    return jsonify(v106_purge_not_real_holdings("수동 정리 요청"))



# ============================================================
# v106 REAL BALANCE DISPLAY HARD FIX
# 반드시 app.run 이전에 배치되어야 하는 최종 보유/감시/매도 보호 로직
# ============================================================

# v106는 과거 v103/v104 보유목록 파일을 사용하지 않습니다.
# 녹십자홀딩스 같은 과거 캐시 종목이 다시 살아나는 것을 막기 위함입니다.
HOLDINGS_FILE = str(BASE_DIR / "sungil_holdings_v106_real_balance.json")
SELL_GUARD_FILE = str(BASE_DIR / "sungil_sell_guard_v106.json")
...
def parse_kiwoom_holdings(data):
    ...
    for arr in v106_deep_lists(data):
        for item in arr:
            ...
            buy = v106_num(first(item, buy_keys), 0)
            cur = v106_num(first(item, cur_keys), 0)
            sellable = v106_num(first(item, sellable_keys), qty)
...
def v106_force_sync_holdings(full_sync=True):
    ...
    if res.get("ok"):
        ...
        write_holdings(items)
        state = read_trade_state()
        state["last_status"] = "v106 키움 실제잔고 full sync 완료"
        state["last_status_time"] = now_kst().strftime("%Y-%m-%d %H:%M:%S")
        ...
        return {
            ...
            "message": "v106: 키움 실제잔고 기준으로 앱 보유목록을 갱신했습니다."
        }
...
def v106_guarded_recent(code, reason="", seconds=3600):
    code = v106_code(code)
    d = v106_load_sell_guard()
    rec = d.get(code)
    if not rec:
        return False
    return time.time() - safe_float(rec.get("time", 0), 0) < seconds

def v106_mark_guard(code, name="", reason=""):
    code = v106_code(code)
    d = v106_load_sell_guard()
    d[code] = {"name": name, "reason": reason, "time": time.time(), "time_text": now_kst().strftime("%Y-%m-%d %H:%M:%S")}
    v106_save_sell_guard(d)

def v106_remove_local_holding(code, name="", reason=""):
    code = v106_code(code)
    items = []
    removed = []
    for h in read_holdings():
        if v106_code(h.get("code")) == code:
            removed.append(h)
        else:
            items.append(h)
    write_holdings(items)
    v106_mark_guard(code, name, reason)
    state = read_trade_state()
    state["last_status"] = "v106 실제 미보유 종목 제거"
    state["last_status_time"] = now_kst().strftime("%Y-%m-%d %H:%M:%S")
    state["last_order_message"] = f"{name or code} 제거: {reason}"
    write_trade_state(state)
    return removed
...
def auto_sell_holding(kind, h, cur):
    ...
    real, res = v106_real_map()
    if res.get("ok"):
        if code not in real:
            v106_remove_local_holding(code, name, "키움 실제잔고에 없는 종목")
            return False
        rh = real[code]
        h.update(rh)
    else:
    ...
    if qty <= 0:
        v106_remove_local_holding(code, name, "매도가능수량 0")
        return False
    ...
    # 최근 동일 오류가 있으면 반복 방지
    if v106_guarded_recent(code, "sell_guard", 3600):
        return False
    ...
    if order.get("ok"):
        ...
        v106_force_sync_holdings(full_sync=True)
        send_telegram_message(
            f"{'✅ 목표가 도달 자동매도' if kind == 'target' else '🛑 손절가 이탈 자동매도'}\n"
            f"종목: <b>{name}</b> ({code})\n"
            f"매수가: {buy:,.0f}원\n"
    ...
    if any(x in order_text for x in ["505217", "장종료", "800033", "매도가능수량", "0주 매도가능"]):
        v106_remove_local_holding(code, name, order.get("message") or order.get("response") or order_text[:300])
        return False

    # 그 외 진짜 주문 오류도 1시간 1회만 알림
    if not v106_guarded_recent(code, "order_fail", 3600):
        v106_mark_guard(code, name, "order_fail")
        send_telegram_message(f"⚠️ <b>자동매도 실패</b>\n종목: {name} ({code})\n사유: {order.get('message') or order.get('response')}")
    return False
...
def watch_loop():
    ...
    while WATCH_STATE.get("running"):
        try:
            sync = v106_force_sync_holdings(full_sync=True)
...
def api_server_holdings_v106():
    if request.method == "POST":
        ...
        if action == "clear":
            write_holdings([])
            return jsonify({"ok": True, "version": "v106", "holdings": [], "message": "v106 로컬 보유목록 초기화 완료"})
        if action == "delete":
            code = v106_code(data.get("code"))
            items = [h for h in read_holdings() if v106_code(h.get("code")) != code]
            write_holdings(items)
...
@app.route("/api/v106_reset_fake_holdings", methods=["GET", "POST"])
def api_v106_reset_fake_holdings():
    """
    배포 직후 1회 실행 권장.
    ...
    for fp in [
        ...
        str(BASE_DIR / "sungil_holdings_v106_real_balance.json"),
    ]:
        try:
    ...
    sync["message"] = "v106 초기화 완료: 과거 보유목록 파일 제거 후 키움 실제잔고로 재동기화했습니다."
    return jsonify(sync)
...
# ============================================================
HOLDINGS_FILE = str(BASE_DIR / "sungil_holdings_v106_real_only.json")
SELL_GUARD_FILE = str(BASE_DIR / "sungil_sell_guard_v106.json")

_REAL_HOLDINGS_CACHE_V106 = {"time": 0, "res": None}
REAL_HOLDINGS_CACHE_SEC_V106 = int(os.getenv("REAL_HOLDINGS_CACHE_SEC", "5"))
...
def parse_kiwoom_holdings(data):
    ...
    by_code = {}
    for item in v106_deep_dicts(data):
        code = v106_code(first(item, code_keys))
        if not code or code == "000000":
            continue
        qty = v106_num(first(item, qty_keys), 0)
        ...
        stop = round(buy * (1 + stop_rate)) if buy else round(cur * (1 + stop_rate))
        buy_amount = buy * qty
        eval_amount = v106_num(first(item, eval_keys), 0) or cur * qty
...
def kiwoom_get_account_holdings():
    """v106: 실제잔고 조회 전용. 실패하면 과거 로컬을 반환하지 않습니다."""
    if not kiwoom_ready():
        return {"ok": False, "message": "키움 환경변수 미설정", "holdings": []}
    if _REAL_HOLDINGS_CACHE_V106.get("res") and time.time() - _REAL_HOLDINGS_CACHE_V106.get("time", 0) < REAL_HOLDINGS_CACHE_SEC_V106:
        return _REAL_HOLDINGS_CACHE_V106["res"]
    ...
    for path, api_id, body in endpoints:
        try:
            ...
            if r.status_code == 200 and holdings:
                ...
                _REAL_HOLDINGS_CACHE_V106["time"] = time.time()
                _REAL_HOLDINGS_CACHE_V106["res"] = res
                update_kiwoom_debug("holdings_ok", "", r.status_code, f"v106 키움 실제잔고 {len(holdings)}개 조회 성공", {"api_id": api_id, "count": len(holdings), "body": body})
                return res
            msg = data.get("return_msg") if isinstance(data, dict) else str(data)
...
def v106_make_holding(code, name, qty, buy):
    code = v106_code(code)
    cur = 0
    src = "SCREEN_FALLBACK"
    try:
    ...
    return {
        ...
        "profitRate": round(((cur - buy) / buy * 100) if buy else 0, 2), "holdingStatus": "스크린샷 보정", "updatedBy": "v106_screen"
    }


def v106_screen_holdings():
    """사용자 제공 2026-05-20 16:10 키움 잔고 화면 기준 비상 보정값."""
    return [
        v106_make_holding("010170", "대한광통신", 11, 23136),
        v106_make_holding("067310", "하나마이크론", 4, 46863),
    ]


def v106_force_sync_holdings(full_sync=True, allow_screen_fallback=False):
    res = kiwoom_get_account_holdings()
    if res.get("ok"):
        items = res.get("holdings", [])
        write_holdings(items)
        state = read_trade_state()
        state["last_status"] = "v106 키움 실제잔고 동기화 완료"
        state["last_status_time"] = now_kst().strftime("%Y-%m-%d %H:%M:%S")
        state["last_order_message"] = f"키움 실제잔고 {len(items)}개 기준으로 표시"
        write_trade_state(state)
        return {"ok": True, "version": "v106", "holdings": items, "count": len(items), "source": "KIWOOM_REAL_BALANCE", "message": "키움 실제잔고 기준으로 보유종목을 동기화했습니다.", "kiwoom": res}

    # 실패 시 과거 로컬/브라우저 보유목록은 절대 표시하지 않음
    if allow_screen_fallback or os.getenv("USE_SCREEN_HOLDINGS_FALLBACK", "false").lower() == "true":
        items = v106_screen_holdings()
        write_holdings(items)
        return {"ok": True, "version": "v106", "holdings": items, "count": len(items), "source": "SCREENSHOT_FALLBACK", "message": "키움 잔고조회 실패로 스크린샷 기준 보유종목을 임시 적용했습니다.", "kiwoom_error": res}

    write_holdings([])
    state = read_trade_state()
    state["last_status"] = "v106 키움 실제잔고 조회 실패"
    state["last_status_time"] = now_kst().strftime("%Y-%m-%d %H:%M:%S")
    state["last_order_message"] = "과거 로컬 보유목록 표시 차단. /api/v106_apply_screen_holdings 로 임시 보정 가능"
    write_trade_state(state)
    return {"ok": False, "version": "v106", "holdings": [], "count": 0, "source": "NONE", "message": "키움 실제잔고 조회 실패. 과거 로컬 보유목록은 표시하지 않습니다.", "kiwoom_error": res}
...
def v106_real_map():
    ...
    return {v106_code(h.get("code")): h for h in items if v106_code(h.get("code")) and safe_float(h.get("qty", 0), 0) > 0}, res


def v106_remove_local_holding(code, name="", reason="실제잔고에 없음"):
    code = v106_code(code)
    items = [h for h in read_holdings() if v106_code(h.get("code")) != code]
    write_holdings(items)
    update_trade_status("v106 실제 미보유 종목 제거", f"{name or code}: {reason}")
    return items


def auto_sell_holding(kind, h, cur):
    """v106: 자동매도는 장중 + 키움 실제잔고 확인 성공 + 실제 보유 종목일 때만 실행."""
    h = normalize_holding(dict(h or {}))
    code = v106_code(h.get("code"))
    name = h.get("name", code)
    if not market_is_open():
    ...
    if not order.get("ok"):
        ...
        if "장종료" in msg or "505217" in msg or "매도가능수량" in msg or "800033" in msg:
            ...
            if "매도가능수량" in msg or "800033" in msg:
                v106_remove_local_holding(code, name, "키움 매도가능수량 0")
            return False
        send_telegram_message(f"⚠️ <b>자동매도 실패</b>\n종목: {name} ({code})\n사유: {msg}")
        return False
    buy = safe_float(h.get("buyPrice", 0))
...
@app.route("/api/v106_force_sync_holdings", methods=["GET", "POST"])
def api_v106_force_sync_holdings():
    return jsonify(v106_force_sync_holdings(full_sync=True))


@app.route("/api/v106_holdings")
def api_v106_holdings():
    return jsonify(v106_force_sync_holdings(full_sync=True))


@app.route("/api/server_holdings_v106", methods=["GET", "POST"])
def api_server_holdings_v106():
    if request.method == "POST":
        data = request.get_json(silent=True) or {}
        action = data.get("action", "")
        if action == "clear":
            write_holdings([])
            return jsonify({"ok": True, "version": "v106", "holdings": [], "message": "로컬 표시목록 초기화. 다음 동기화 때 키움 실제잔고만 표시됩니다."})
        if action == "delete":
            # 실제잔고 기준 앱이므로 삭제는 임시 화면 삭제일 뿐입니다.
            code = v106_code(data.get("code"))
            items = [h for h in read_holdings() if v106_code(h.get("code")) != code]
            write_holdings(items)
            return jsonify({"ok": True, "version": "v106", "holdings": items, "message": "화면에서 임시 제거했습니다. 키움에 실제 보유 중이면 다시 동기화됩니다."})
    return jsonify(v106_force_sync_holdings(full_sync=True))


@app.route("/api/restore_holdings", methods=["POST"])
def api_restore_holdings_v106_disabled():
    # 브라우저 백업이 과거 종목을 되살리는 원인을 차단합니다.
    return jsonify(v106_force_sync_holdings(full_sync=True))


@app.route("/api/v106_apply_screen_holdings", methods=["GET", "POST"])
def api_v106_apply_screen_holdings():
    items = v106_screen_holdings()
    write_holdings(items)
    state = read_trade_state()
    state["last_status"] = "v106 스크린샷 기준 임시 보유목록 적용"
    state["last_status_time"] = now_kst().strftime("%Y-%m-%d %H:%M:%S")
    state["last_order_message"] = "대한광통신 11주, 하나마이크론 4주 임시 반영. 키움 실제잔고 API 정상화 시 자동 대체됩니다."
    write_trade_state(state)
    return jsonify({"ok": True, "version": "v106", "holdings": items, "message": "스크린샷 기준 실제 보유종목을 임시 반영했습니다."})


@app.route("/api/v106_reset_all_holdings", methods=["GET", "POST"])
def api_v106_reset_all_holdings():
    removed_files = []
    for fp in [
        ...
        str(BASE_DIR / "sungil_holdings_v101.json"), str(BASE_DIR / "sungil_holdings_v102.json"), str(BASE_DIR / "sungil_holdings_v103.json"), str(BASE_DIR / "sungil_holdings_v104.json"), str(BASE_DIR / "sungil_holdings_v105_real_balance.json"), str(BASE_DIR / "sungil_holdings_v106_real_only.json"),
    ]:
        try:
            if os.path.exists(fp):
                os.remove(fp)
            ...
            pass
    _REAL_HOLDINGS_CACHE_V106["time"] = 0
    _REAL_HOLDINGS_CACHE_V106["res"] = None
    res = v106_force_sync_holdings(full_sync=True)
