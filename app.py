# -*- coding: utf-8 -*-
"""
성일의 AI 주식바람 - KIWOOM REAL AUTO SCALPING v102 REAL HOLDINGS DISPLAY FIX
파일명: app_kiwoom_real_auto_scalping_v102_real_holdings_display_fix.py

이 파일은 사용자가 업로드한 v102 계열 전체 텍스트를 기반으로 v102 목표 기능을 반영한 업그레이드본입니다.

v102 반영 핵심:
- 기존 키움 REST 토큰/현재가/주문/잔고동기화 구조 유지
- 기존 보유종목 자동등록/Render 재배포 후 복구/localStorage 백업 구조 유지
- AI 단타 후보에 v102 스캘핑 점수 추가
- 거래량 급증/거래대금/등락률/테마가중/체결강도 추정/호가잔량 추정 점수화
- 매도 후 자동 신규 후보 탐색/재매수 구조 유지 및 상태 표시 강화
- 실시간 상태 API /api/v102_dashboard 추가
- 기존 /api/version을 v102로 갱신
...
HOLDINGS_FILE=os.getenv('SERVER_HOLDINGS_FILE','/tmp/sungil_holdings_v102.json')
...
# v102 스캘핑 AI 확장 엔진
# =========================================================
def v102_clamp(v, lo=0, hi=100):
    try:
        return max(lo, min(hi, safe_float(v, 0)))
    except Exception:
        return lo

def v102_get_orderbook_metrics(code):
    """
    호가잔량/매수압력 조회.
...
def v102_estimate_execution_strength(row):
    """
    ...
    return round(v102_clamp(base, 50, 200), 2)

def v102_calculate_scalping_score(row, price, orderbook=None):
    """
    v102 최종 스캘핑 점수.
    - 기존 후보 점수
    - 거래량 급증
    ...
    # 거래량/거래대금 급증 점수
    volume_spike_score = v102_clamp(volume_rank * 0.6 + amount_rank * 0.4, 0, 100)

    # 체결강도
    execution_strength = v102_estimate_execution_strength(row)
    ...
    if bid_ask_ratio >= 1.8:
        ...
        orderbook_score = v102_clamp((volume_rank + amount_rank) / 2 * 0.75, 20, 75)
    ...
    final_score = round(v102_clamp(raw, 0, 100), 2)
...
def score_candidates(limit=700,cash=500000,min_qty=5,max_change=7,min_amount=1000000000,min_score=70):
    ...
    for _,row in df.head(20).iterrows():
        ...
        if qty < min_qty:
            continue
        orderbook = v102_get_orderbook_metrics(code)
        v102_ai = v102_calculate_scalping_score(row, p, orderbook)
        base_pick = {'code':code,'name':str(row['Name']),'market':str(row.get('Market','')),'theme':normalize_theme(row['theme']),'price':round(p),'priceSource':src,'score':v102_ai['aiScoreV102'],'dayChange':round(safe_float(row['dayChange']),2),'amount':round(safe_float(row['Amount'])),'qtyPossible':qty,'buyZone':round(p*.995),'target':round(p*1.035),'stop':round(p*.975),'comment':f"v102 스캘핑 AI: {v102_ai['scalpingStatus']} · {', '.join(v102_ai['scalpingReasons'])}. 현재가는 {src} 기준입니다. 추격보다 호가·거래량 유지 확인 후 접근이 좋습니다."}
        base_pick.update(v102_ai)
...
def best_pick_from_params(args=None):
    ...
    picks=sorted(picks, key=lambda x: safe_float(x.get('orderPriority', x.get('aiScoreV102', x.get('score', 0)))), reverse=True)
...
TRADE_STATE_FILE = os.getenv("TRADE_STATE_FILE", "/tmp/sungil_trade_state_v102.json")
...
TRADE_DEFAULTS = {
    ...
    # v102 cash multi fix: 화면 입력 총투자금이 아니라 키움 예수금/주문가능금액을 우선 사용합니다.
...
# ===============================
# v102 REAL HOLDINGS DISPLAY FIX: 키움 예수금/주문가능금액 기반 매수금 계산
# ===============================



# ============================================================
# v102 키움 예수금 qry_tp 필수 파라미터 보정
# ============================================================
...
def normalize_kiwoom_cash_result(data):
    """
    v102 보강: 여러 응답 필드명에서 예수금/주문가능금액을 더 넓게 탐색합니다.
    """
    try:
        orderable_keys = [
...
# ============================================================
# v102 AI 추천 진입금액 / 보유종목 상세 표시 모듈
# ============================================================
def v102_ai_position_rate(ai_score=80, price=0, orderable_cash=0, current_positions=0, max_positions=3):
    """
    AI 단타 최적화용 진입 비율.
...
def v102_calc_ai_recommended_budget(pick=None, live_price=0):
    """
    v102 핵심:
    ...
    if isinstance(pick, dict):
        ai_score = safe_float(
            pick.get("aiScoreV102", pick.get("aiScoreV90", pick.get("score", pick.get("orderPriority", 80)))),
            80
        )

    price = safe_float(live_price or (pick.get("price", 0) if isinstance(pick, dict) else 0), 0)
    rate = v102_ai_position_rate(ai_score, price, usable_cash, current_positions, max_positions)
...
def v102_calc_order_qty_from_ai_budget(pick=None, live_price=0):
    budget, info = v102_calc_ai_recommended_budget(pick, live_price)
    price = safe_float(live_price, 0)
...
def v102_holding_status(rate, cur=0, target=0, stop=0):
...
def v102_enrich_holding(h):
    ...
    h["holdingStatus"] = v102_holding_status(rate, cur, target, stop)
    h["aiComment"] = ai_comment(cur, buy, target, stop, qty)
    h["updatedBy"] = "v102"
    return h


def v102_get_enriched_holdings():
    items = read_holdings()
    enriched = []
    for h in items:
        try:
            enriched.append(v102_enrich_holding(h))
        except Exception as e:
            hh = dict(h)
            hh["v102Error"] = str(e)
            enriched.append(hh)
...
try:
    # [v102 duplicate route disabled] @app.route("/api/v102_holdings")
    def api_v102_holdings():
        holdings = v102_get_enriched_holdings()
        ...
        return jsonify({
            ...
            "message": "v102 보유종목 상세 조회: 매수가/현재가/수량/매수금액/평가금액/손익/목표가/손절가/상태/AI코멘트"
        })
except Exception:
    pass

try:
    @app.route("/api/v102_ai_budget")
    def api_v102_ai_budget():
        ...
        budget, info = v102_calc_ai_recommended_budget(pick, price)
        return jsonify({
            "ok": info.get("ok"),
            "version": "v102",
            "budget": int(budget),
...
# ============================================================
# v102 주문 가능 수량 0 오류 수정 모듈
# ============================================================
def v102_extract_orderable_cash(info=None):
    info = info or {}
    vals = []
...
def v102_get_orderable_cash():
    try:
        if "get_trade_cash_info" in globals():
            res = get_trade_cash_info()
            c = v102_extract_orderable_cash(res)
            if c > 0:
                return c, res
    except Exception as e:
        last_err = str(e)
...
def v102_calc_final_order_qty(pick=None, live_price=0):
    state = read_trade_state()
    price = safe_float(live_price or (pick.get("price", 0) if isinstance(pick, dict) else 0), 0)
    ...
    orderable_cash, cash_raw = v102_get_orderable_cash()
    ...
    return qty, {
        "ok": qty > 0,
        "label": "AI 추천 진입금액",
        "message": "v102 주문가능금액 기준 최종 수량 계산 완료" if qty > 0 else "최종 주문수량 0",
        "orderable_cash": int(orderable_cash),
        "kiwoom_orderable_cash": int(orderable_cash),
        "price": int(price),
...
def v102_calc_order_qty_from_ai_budget(pick=None, live_price=0):
    return v102_calc_final_order_qty(pick, live_price)
...
try:
    # [v102 duplicate route disabled] @app.route("/api/v102_order_qty_test")
    def api_v102_order_qty_test():
        price = safe_float(request.args.get("price", 0), 0)
        score = safe_float(request.args.get("score", 80), 80)
        code = request.args.get("code", "")
        pick = {"code": code, "price": price, "score": score}
        qty, info = v102_calc_final_order_qty(pick, price)
        return jsonify({"ok": qty > 0, "version": "v102", "qty": qty, "info": info})
except Exception:
    pass
...
# ============================================================
# v102 주문가능금액 적극 활용 / 키움 실제잔고 강제동기화 보강
# ============================================================
def v102_calc_aggressive_ai_budget(pick=None, live_price=0):
    """
    v102 핵심:
    기존 v96/v97은 최소진입금 5만원 근처로 너무 작게 진입하는 문제가 있었습니다.
    v102은 키움 주문가능금액과 최대 동시보유 종목수를 기준으로 남은 예수금을 최대한 활용합니다.
    ...
    return recommended, {
        "ok": qty > 0,
        "label": "AI 추천 진입금액",
        "message": "v102 주문가능금액 적극 활용 기준 AI 추천 진입금액 계산 완료",
        "orderable_cash": int(orderable_cash),
        "kiwoom_orderable_cash": int(orderable_cash),
        "price": int(price),
        "max_positions": max_positions,
...
def v102_calc_final_order_qty(pick=None, live_price=0):
    budget, info = v102_calc_aggressive_ai_budget(pick, live_price)
    price = safe_float(live_price or (pick.get("price", 0) if isinstance(pick, dict) else 0), 0)
    if not info.get("ok") or price <= 0:
        return 0, info
...
# 기존 함수명 완전 덮어쓰기: 이후 코드가 어떤 버전 함수를 호출해도 v102 계산 사용
...
def v102_force_sync_holdings_old_disabled(full_sync=True):
    """
    보유종목 강제 새로고침 전용.
    키움 실제잔고를 기준으로 앱 로컬 보유종목을 완전히 덮어씁니다.
    """
    try:
        if "v102_force_sync_holdings" in globals():
            res = v102_force_sync_holdings(full_sync=full_sync)
        else:
            res = {"ok": False, "message": "v102_force_sync_holdings 함수 없음"}
        ...
        if res.get("ok"):
            ...
            res["holdings"] = fixed
            res["version"] = "v102"
            res["message"] = "v102: 키움 실제잔고 기준으로 수량/매입가/현재가를 강제 동기화했습니다."
        return res
    except Exception as e:
        return {"ok": False, "version": "v102", "message": str(e), "holdings": read_holdings()}


# 기존 이름도 v102로 덮어쓰기
def sync_kiwoom_holdings_to_local():
    res = v102_force_sync_holdings(full_sync=True)
    return res.get("holdings", read_holdings())


try:
    # [v102 duplicate route disabled] @app.route("/api/v102_force_sync_holdings", methods=["GET", "POST"])
    def api_v102_force_sync_holdings():
        full = str(request.args.get("full", "1")).lower() not in ["0", "false", "no"]
        return jsonify(v102_force_sync_holdings(full_sync=full))
except Exception:
    pass


try:
    @app.route("/api/v102_order_qty_test")
    def api_v102_order_qty_test_dup2():
        price = safe_float(request.args.get("price", 0), 0)
        score = safe_float(request.args.get("score", 80), 80)
        code = request.args.get("code", "")
        pick = {"code": code, "price": price, "score": score}
...
def try_rebuy_after_sell(sold_code=""):
    ...
    try:
        ...
        # v102 REAL HOLDINGS DISPLAY FIX: 기존 보유종목이 있어도 최대 보유종목 수와 예수금이 허용하면 신규 후보를 추가 매수합니다.
...
def trade_can_buy_v102(code, price):
    """
    v102 매수 가능 조건:
    - 1종목 제한 제거
    - 최대 동시 보유 종목수까지만 허용
...
try:
    ...
    def api_v102_cash():
        ...
        return jsonify({
            ...
            "message": "v102는 앱 입력금액이 아니라 키움 주문가능금액을 자동 조회하여 매수 기준으로 사용합니다."
        })
except Exception:
    pass
...
# ============================================================
# v102 자동매수 함수 보강
# 기존 auto_buy_best_pick이 있으면 아래 함수가 이름을 덮어써서 v102 로직을 사용합니다.
# ============================================================
...
def auto_buy_best_pick(args=None, use_latest_ui_pick=False):
    ...
    update_trade_status("종목 탐색중", "v102 키움 주문가능금액 기준으로 AI 단타 후보를 찾는 중입니다.")
    ...
    allowed, reason = trade_can_buy_v102(code, live)

    if not allowed:
        update_trade_status("매수 보류", reason, candidate=pick)
        return {"ok": False, "message": reason, "pick": pick}

    qty, budget = v102_calc_order_qty_from_ai_budget(pick, live)

    if qty <= 0:
        reason = budget.get("message", "v102 계산 결과 주문 가능 수량이 0입니다.") if isinstance(budget, dict) else "v102 계산 결과 주문 가능 수량이 0입니다."
        update_trade_status("매수 보류", reason, candidate=pick)
        return {"ok": False, "message": reason, "pick": pick, "budget": budget}

    update_trade_status(
        "주문 전송중",
        f"v102 키움 주문가능금액 기준 매수 주문: {pick.get('name')} {qty}주 / 주문가능금액 {budget.get('orderable_cash', 0):,.0f}원 / AI추천예산 {budget.get('final_order_budget', budget.get('ai_recommended_budget', budget.get('budget', 0))):,.0f}원",
        candidate=pick
    )
    ...
    if order.get("ok"):
        ...
        update_trade_status(
            "매수 성공" if not order.get("dry_run") else "DRY-RUN 매수 성공",
            f"v102 {pick['name']} {qty}주 매수 처리 완료",
            candidate=pick,
            order=order
        )

        send_trade_telegram(
            f"🚀 <b>v102 AI 자동매수 {'DRY-RUN ' if order.get('dry_run') else ''}진행</b>\n"
            f"종목: <b>{pick['name']}</b> ({code})\n"
            f"매수가 기준: {live:,.0f}원 ({price_src})\n"
            f"수량: {qty:,}주\n"
            f"매수금액: {buy_amount:,.0f}원\n"
            ...
            f"테마: {pick.get('theme', '')}\n"
            f"시간: {now_kst().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
            "※ v102는 앱 입력금액이 아니라 키움 주문가능금액을 자동 기준으로 사용합니다.",
            "buy_success_v102"
        )
...
# [v102 duplicate route disabled] @app.route('/api/server_holdings_v102',methods=['GET','POST'])
def api_server_holdings():
    if request.method=='GET':
        ...
        if request.args.get('refresh') == '1':
            sync_res = v102_force_sync_holdings(full_sync=True) if 'v102_force_sync_holdings' in globals() else {'holdings': holdings}
...
@app.route('/api/v102_dashboard')
def api_v102_dashboard():
    """
    v102 실시간 수익률/후보/상태 통합 대시보드 API.
    ...
    try:
        ...
        return jsonify(safe_json({
            "ok": True,
            "version": "KIWOOM REAL AUTO SCALPING v102 REAL HOLDINGS DISPLAY FIX",
            "time": now_kst().strftime("%Y-%m-%d %H:%M:%S"),
            "summary": {
...
def api_version(): return jsonify({'ok':True,'version':'kiwoom-real-auto-scalping-v102-upgrade','watch_interval':WATCH_INTERVAL,'file':'app_kiwoom_real_auto_scalping_v102_fetch_fix.py','v102_dashboard':'/api/v102_dashboard'})

HTML = r'''<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>성일의 AI 주식바람 v102</title><style>
...
function go(id){document.getElementById(id).scrollIntoView({behavior:"smooth"})}function getParams(){return new URLSearchParams({priceRanges:[...$("priceRanges").selectedOptions].map(o=>o.value).join(","),cash:num($("cash").value),minQty:num($("minQty").value),maxChange:num($("maxChange").value),minAmount:num($("minAmount").value),minScore:num($("minScore").value)})}async function fetchJson(url,opts={}){const c=new AbortController(),timeoutMs=Number(opts.timeoutMs||120000),t=setTimeout(()=>c.abort(),timeoutMs);try{const r=await fetch(url,{...opts,cache:"no-store",headers:{Accept:"application/json",...(opts.headers||{})},signal:c.signal});const txt=await r.text();if(!r.ok){throw new Error(`서버 오류 ${r.status}: ${txt.slice(0,160)}`)}try{return JSON.parse(txt)}catch(e){throw new Error("서버가 JSON이 아닌 응답을 반환했습니다.")}}catch(e){if(e.name==="AbortError") throw new Error("요청 시간이 길어져 중단되었습니다. 잠시 후 다시 시도하거나 후보 조건을 낮춰주세요.");throw e}finally{clearTimeout(t)}}function renderPick(p){if(!p)return"<div class='empty'>조건에 맞는 단타 후보가 없습니다. 조건을 낮춰보세요.</div>";return`<div class="pick"><div class="meta"><span>${p.market}</span><span>${p.code}</span><span>${p.theme}</span><span>AI ${p.score}</span></div><h2>${p.name}</h2><div class="metrics"><div class="metric"><small>현재가</small><b>${fmt(p.price)}</b><br><small>${p.priceSource||"-"}</small></div><div class="metric"><small>당일 흐름</small><b>${p.dayChange}%</b></div><div class="metric"><small>거래대금</small><b>${(p.amount/100000000).toFixed(1)}억</b></div><div class="metric"><small>매수관찰</small><b>${fmt(p.buyZone)}</b></div><div class="metric"><small>목표가</small><b class="red">${fmt(p.target)}</b></div><div class="metric"><small>손절가</small><b class="blue">${fmt(p.stop)}</b></div></div><div class="comment">AI 코멘트: ${p.comment}</div></div>`}async function loadBest(){$("bestBox").innerHTML="조회중...";try{const d=await fetchJson("/api/best_pick?"+getParams().toString(),{timeoutMs:120000});$("bestBox").innerHTML=renderPick(d.pick)}catch(e){$("bestBox").innerHTML="<div class='empty'>조회 오류: "+e.message+"</div>"}}async function loadWatch(){$("watchBox").innerHTML="조회중...";try{const d=await fetchJson("/api/watch_candidates?"+getParams().toString(),{timeoutMs:120000});$("watchBox").innerHTML=(d.items||[]).map(renderPick).join("")||"<div class='empty'>감시 후보가 없습니다.</div>"}catch(e){$("watchBox").innerHTML="<div class='empty'>조회 오류: "+e.message+"</div>"}}async function testBetterAlert(){const d=await fetchJson("/api/best_pick/test_alert?"+getParams().toString());alert(d.ok?"텔레그램 후보 알림 발송 완료":(d.message||"발송 실패"))}async function findCode(){const name=$("hName").value.trim();if(!name||$("hCode").value.trim())return;try{const d=await fetchJson("/api/find_stock?q="+encodeURIComponent(name));if(d.ok){$("hCode").value=d.code;if(!$("hBuy").value&&d.price)$("hBuy").value=Math.round(d.price);calcHolding()}}catch(e){}}function calcHolding(){const buy=num($("hBuy").value),amount=num($("hAmount").value);if(buy&&amount&&!$("hQty").value)$("hQty").value=Math.floor(amount/buy);if(buy&&!$("hTarget").value)$("hTarget").value=Math.round(buy*1.035);if(buy&&!$("hStop").value)$("hStop").value=Math.round(buy*.975)}async function addHolding(){await findCode();calcHolding();const item={name:$("hName").value.trim(),code:$("hCode").value.trim(),buyPrice:num($("hBuy").value),buyAmount:num($("hAmount").value),qty:num($("hQty").value),target:num($("hTarget").value),stop:num($("hStop").value)};if(!item.name||!item.code||!item.buyPrice){alert("종목명, 종목코드, 매수가는 필수입니다.");return}await fetchJson("/api/server_holdings_v102",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"add",item})});await refreshHoldings()}async function refreshHoldings(){const d=await fetchJson("/api/v102_force_sync_holdings?full=1",{method:"POST",timeoutMs:120000});renderHoldings(d.holdings||[])}async function clearHoldings(){if(!confirm("보유종목을 모두 삭제할까요?"))return;const d=await fetchJson("/api/server_holdings_v102",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"clear"})});renderHoldings(d.holdings||[])}async function loadHoldings(autoRestore=true){
  ...
  let d={holdings:[]};
  try{
    d=await fetchJson("/api/server_holdings_v102",{timeoutMs:60000});
  }catch(e){
  ...
  if(list.length){
    setTimeout(async()=>{
      try{
        const rd=await fetchJson("/api/server_holdings_v102",{
          method:"POST",
          headers:{"Content-Type":"application/json"},
...
async function removeHolding(id,code){const d=await fetchJson("/api/server_holdings_v102",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"remove",id,code})});renderHoldings(d.holdings||[])}
...
// v102 FIX: 아래 중복 loadHoldings 함수가 기존 복구/백업 로직을 덮어써서
...
# v102: Render 배포/재시작 시 Logs에 서버 공인 IP 출력
try:
...
# v102: 화면에서 키움 예수금 실제 조회 상태 확인용
try:
    @app.route("/api/v102_cash_check")
    def api_v102_cash_check():
        try:
        ...
        return jsonify({
            ...
            "message": "v102: qry_tp 필수 파라미터 포함 후 키움 예수금/주문가능금액 조회 결과입니다."
        })
except Exception:
    pass
...
def v102_price_test(code="005930"):
...
def v102_kiwoom_diagnosis(code="005930"):
    try:
    ...
    env = v102_env_status()
    token = v102_token_test()
    price = v102_price_test(code)
    cash = v102_cash_test()
...
try:
    @app.route("/api/v102_kiwoom_diag")
    def api_v102_kiwoom_diag():
        code = request.args.get("code", "005930")
        return jsonify(v102_kiwoom_diagnosis(code))
except Exception:
    pass


try:
    @app.route("/api/v102_price_test")
    def api_v102_price_test():
        code = request.args.get("code", "005930")
        return jsonify(v102_price_test(code))
except Exception:
    pass


try:
    @app.route("/api/v102_cash_test")
    def api_v102_cash_test():
        return jsonify(v102_cash_test())
except Exception:
    pass
...
# ============================================================
# v102 키움 실제잔고 강제 동기화 모듈
# ============================================================
def v102_normalize_real_holding(h):
    """
    ...
    return {
        ...
        "syncSource": "KIWOOM_FORCE_SYNC_V102",
        "updatedBy": "v102"
    }


def v102_get_kiwoom_real_holdings_safe():
    """
...
def v102_force_sync_holdings(full_sync=True):
    """
    v102 핵심:
    ...
    - 앱에만 있는 종목은 유지
    """
    real = v102_get_kiwoom_real_holdings_safe()
    if not real.get("ok"):
        state = read_trade_state()
...
# 기존 동기화 함수명을 v102 방식으로 덮어쓰기
def sync_kiwoom_holdings_to_local():
    return v102_force_sync_holdings(full_sync=False).get("holdings", read_holdings())


try:
    # [v102 duplicate route disabled] @app.route("/api/v102_force_sync_holdings", methods=["GET", "POST"])
    def api_v102_force_sync_holdings_dup2():
        full = str(request.args.get("full", "1")).lower() not in ["0", "false", "no"]
        return jsonify(v102_force_sync_holdings(full_sync=full))
except Exception:
    pass
...
# ============================================================
# v102 최종 보유종목 키움 실제잔고 강제동기화 모듈
# ============================================================
def v102_clean_code(raw):
    s = str(raw or "").strip().replace("A", "")
    s = re.sub(r"[^0-9]", "", s)
    return s.zfill(6) if s else ""


def v102_num(v, default=0):
    try:
        if v is None:
            return default
        if isinstance(v, str):
...
def v102_find_holding_lists(obj):
    """
...
def parse_kiwoom_holdings(data):
    ...
    by_code = {}
    for arr in v102_find_holding_lists(data):
        for item in arr:
            if not isinstance(item, dict):
                continue

            code = v102_clean_code(get_first(item, code_keys))
            if not code or code == "000000":
                continue

            qty = v102_num(get_first(item, qty_keys), 0)
            if qty <= 0:
                continue

            name = str(get_first(item, name_keys) or code).strip()
            buy = v102_num(get_first(item, buy_keys), 0)
            cur = v102_num(get_first(item, cur_keys), 0)
            ...
            holding = {
                ...
                "lastCheckedAt": now_kst().strftime("%Y-%m-%d %H:%M:%S"),
                "highPrice": round(cur),
                "syncSource": "KIWOOM_REAL_BALANCE_V102",
                "updatedBy": "v102"
            }
...
# 기존 함수명도 v102 full sync로 덮어쓰기
def sync_kiwoom_holdings_to_local():
    return v102_force_sync_holdings(full_sync=True).get("holdings", read_holdings())


try:
    # [v102 duplicate route disabled] @app.route("/api/v102_force_sync_holdings", methods=["GET", "POST"])
    def api_v102_force_sync_holdings_dup2_dup2():
        full = str(request.args.get("full", "1")).lower() not in ["0", "false", "no"]
        return jsonify(v102_force_sync_holdings(full_sync=full))
except Exception:
    pass
...
try:
    # [v102 duplicate route disabled] @app.route("/api/v102_holdings")
    def api_v102_holdings_dup2_dup2():
        refresh = str(request.args.get("refresh", "0")).lower() in ["1", "true", "yes"]
        if refresh:
            return jsonify(v102_force_sync_holdings(full_sync=True))
        items = read_holdings()
...
def parse_kiwoom_holdings(data):
    ...
    by_code = {}
    for arr in v102_deep_lists(data):
        for item in arr:
            if not isinstance(item, dict):
                continue
            code = v102_code(first(item, code_keys))
            if not code or code == "000000":
                continue

            qty = v102_num(first(item, qty_keys), 0)
            if qty <= 0:
                continue

            name = str(first(item, name_keys) or code).strip()
            buy = v102_num(first(item, buy_keys), 0)
            cur = v102_num(first(item, cur_keys), 0)
            src = "KIWOOM_ACCOUNT"
...
def v102_force_sync_holdings(full_sync=True):
    ...
    if not res.get("ok"):
        ...
        state["last_order_message"] = str(res.get("message", ""))[:500]
        write_trade_state(state)
        return {"ok": False, "version": "v102", "message": "키움 실제잔고 조회 실패", "detail": res, "holdings": read_holdings()}
    ...
    return {"ok": True, "version": "v102", "api_id": res.get("api_id"), "full_sync": True, "count": len(real_items), "holdings": real_items, "message": "키움 실제잔고 기준으로 앱 보유종목을 완전 동기화했습니다."}
...
def api_v102_holdings_dup2():
    ...
    if refresh:
        return jsonify(v102_force_sync_holdings(full_sync=True))
    return jsonify({"ok": True, "version": "v102", "holdings": read_holdings()})


# [v102 duplicate route disabled] @app.route("/api/server_holdings_v102", methods=["GET", "POST"])
def api_server_holdings_v102():
    if request.method == "POST":
        ...
        if action == "delete":
            code = v102_code(data.get("code"))
            items = [h for h in read_holdings() if v102_code(h.get("code")) != code]
...
# ============================================================
# v102 최종 라우트: 보유종목 강제동기화 단일 진입점
# ============================================================
# [v102 duplicate route disabled] @app.route("/api/v102_force_sync_holdings", methods=["GET", "POST"])
def api_v102_force_sync_holdings_final():
    fn = globals().get("v102_force_sync_holdings") or globals().get("v100_force_sync_holdings") or globals().get("v99_force_sync_holdings") or globals().get("v98_force_sync_holdings")
    if fn:
    ...
    if "kiwoom_get_account_holdings" in globals():
        ...
        if res.get("ok"):
            ...
            return jsonify({"ok": True, "version": "v102", "holdings": res.get("holdings", []), "message": "키움 실제잔고 기준 동기화 완료"})
    return jsonify({"ok": False, "version": "v102", "message": "force sync function not found", "holdings": read_holdings()})


# [v102 duplicate route disabled] @app.route("/api/v102_holdings")
def api_v102_holdings_final():
    refresh = str(request.args.get("refresh", "0")).lower() in ["1", "true", "yes"]
    if refresh:
        return api_v102_force_sync_holdings_final()
    return jsonify({"ok": True, "version": "v102", "holdings": read_holdings()})


# [v102 duplicate route disabled] @app.route("/api/server_holdings_v102", methods=["GET", "POST"])
def api_server_holdings_v102_final():
    if request.method == "POST":
        data = request.get_json(silent=True) or {}
        action = data.get("action", "")
        if action in ["refresh", "force_refresh", "sync", "kiwoom_sync"]:
...
# ============================================================
# v102 실제 키움잔고 표시 우선 / 오래된 보유목록 차단
# ============================================================
def v102_force_sync_holdings(full_sync=True):
    """
        ...
        if fn:
            try:
                ...
                if isinstance(res, dict) and res.get("ok"):
                    ...
                    res["message"] = "v102: 키움 실제잔고 기준으로 서버 보유목록을 완전 동기화했습니다."
                    return res
            except Exception as e:
                last_error = str(e)
    ...
    try:
        ...
        if res.get("ok"):
            ...
            return {
                ...
                "message": "v102: 키움 실제잔고를 직접 조회하여 서버 보유목록을 덮어썼습니다."
            }
        return {
            "ok": False,
            "version": "v102",
...
@app.route("/api/v102_force_sync_holdings", methods=["GET", "POST"])
def api_v102_force_sync_holdings_final():
    return jsonify(v102_force_sync_holdings(full_sync=True))


@app.route("/api/v102_holdings")
def api_v102_holdings_final():
...
@app.route("/api/server_holdings_v102", methods=["GET", "POST"])
def api_server_holdings_v102_final():
    """
    보유 탭 전용 API.
    ...
    if request.method == "POST":
        ...
        if action == "clear":
            write_holdings([])
            return jsonify({"ok": True, "version": "v102", "holdings": [], "message": "앱 로컬 보유목록 초기화 완료. 키움 동기화 시 실제잔고로 복구됩니다."})
        if action == "delete":
            code = str(data.get("code", "")).replace("A", "").zfill(6)
            items = [h for h in read_holdings() if str(h.get("code", "")).replace("A", "").zfill(6) != code]
            write_holdings(items)
