...
try:
    ...
    def api_v109_order_qty_test():
        ...
        return jsonify({"ok": qty > 0, "version": "v109", "qty": qty, "info": info})
except Exception:
    pass
...
def v109_calc_aggressive_ai_budget(pick=None, live_price=0):
    ...
    v109은 키움 주문가능금액과 최대 동시보유 종목수를 기준으로 남은 예수금을 최대한 활용합니다.
...
def v109_force_sync_holdings_old_disabled(full_sync=True):
    ...
    try:
        ...
        return {"ok": False, "version": "v109", "message": str(e), "holdings": read_holdings()}
...
try:
    ...
    def api_v109_cash():
        ...
        return jsonify({
            ...
            "message": "v109는 앱 입력금액이 아니라 키움 주문가능금액을 자동 조회하여 매수 기준으로 사용합니다."
        })
except Exception:
    pass
...
def auto_buy_best_pick(args=None, use_latest_ui_pick=False):
    ...
    if order.get("ok"):
        ...
        send_trade_telegram(
            f"🚀 <b>v109 AI 자동매수 {'DRY-RUN ' if order.get('dry_run') else ''}진행</b>\n"
            f"종목: <b>{pick['name']}</b> ({code})\n"
            f"매수가 기준: {live:,.0f}원 ({price_src})\n"
            f"수량: {qty:,}주\n"
            f"매수금액: {buy_amount:,.0f}원\n"
            ...
            f"테마: {pick.get('theme', '')}\n"
            f"시간: {now_kst().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
            "※ v109는 앱 입력금액이 아니라 키움 주문가능금액을 자동 기준으로 사용합니다.",
            "buy_success_v109"
        )
...
try:
    ...
    def api_v109_holdings_dup2():
        ...
        return jsonify({"ok": True, "version": "v109", "holdings": items})
except Exception:
    pass
...
def v109_force_sync_holdings(full_sync=True):
    ...
    if not res.get("ok"):
        ...
        state["last_order_message"] = str(res.get("message", ""))[:500]
        write_trade_state(state)
        return {"ok": False, "version": "v109", "message": "키움 실제잔고 조회 실패", "detail": res, "holdings": read_holdings()}
    ...
    return {"ok": True, "version": "v109", "api_id": res.get("api_id"), "full_sync": True, "count": len(real_items), "holdings": real_items, "message": "키움 실제잔고 기준으로 앱 보유종목을 완전 동기화했습니다."}
...
def api_v109_holdings_dup2():
    ...
    if refresh:
        return jsonify(v109_force_sync_holdings(full_sync=True))
    return jsonify({"ok": True, "version": "v109", "holdings": read_holdings()})
...
def api_v109_force_sync_holdings_final():
    ...
    if "kiwoom_get_account_holdings" in globals():
        ...
        if res.get("ok"):
            ...
            return jsonify({"ok": True, "version": "v109", "holdings": res.get("holdings", []), "message": "키움 실제잔고 기준 동기화 완료"})
    return jsonify({"ok": False, "version": "v109", "message": "force sync function not found", "holdings": read_holdings()})
...
def api_server_holdings_v109_final():
    ...
    if request.method == "POST":
        ...
        if action == "clear":
            write_holdings([])
            return jsonify({"ok": True, "version": "v109", "holdings": [], "message": "앱 로컬 보유목록 초기화 완료. 키움 동기화 시 실제잔고로 복구됩니다."})
        if action == "delete":
            code = str(data.get("code", "")).replace("A", "").zfill(6)
            items = [h for h in read_holdings() if str(h.get("code", "")).replace("A", "").zfill(6) != code]
            write_holdings(items)
...
def api_server_holdings_v109_final():
    if request.method == "POST":
        ...
        if action == "clear":
            write_holdings([])
            return jsonify({"ok": True, "version": "v109", "holdings": [], "message": "앱 로컬 보유목록 초기화 완료"})
        if action == "delete":
            code = v109_code(data.get("code"))
            items = [h for h in read_holdings() if v109_code(h.get("code")) != code]
            write_holdings(items)
...
def api_server_holdings_v109():
    if request.method == "POST":
        ...
        if action == "clear":
            write_holdings([])
            return jsonify({"ok": True, "version": "v109", "holdings": [], "message": "v109 로컬 보유목록 초기화 완료"})
        if action == "delete":
            code = v109_code(data.get("code"))
            items = [h for h in read_holdings() if v109_code(h.get("code")) != code]
            write_holdings(items)
...
def v109_force_sync_holdings(full_sync=True, allow_screen_fallback=False):
    ...
    if res.get("ok"):
        ...
        write_trade_state(state)
        return {"ok": True, "version": "v109", "holdings": items, "count": len(items), "source": "KIWOOM_REAL_BALANCE", "message": "키움 실제잔고 기준으로 보유종목을 동기화했습니다.", "kiwoom": res}
    ...
    if allow_screen_fallback or os.getenv("USE_SCREEN_HOLDINGS_FALLBACK", "false").lower() == "true":
        ...
        write_holdings(items)
        return {"ok": True, "version": "v109", "holdings": items, "count": len(items), "source": "SCREENSHOT_FALLBACK", "message": "키움 잔고조회 실패로 스크린샷 기준 보유종목을 임시 적용했습니다.", "kiwoom_error": res}
    ...
    write_trade_state(state)
    return {"ok": False, "version": "v109", "holdings": [], "count": 0, "source": "NONE", "message": "키움 실제잔고 조회 실패. 과거 로컬 보유목록은 표시하지 않습니다.", "kiwoom_error": res}
...
def api_server_holdings_v109():
    if request.method == "POST":
        ...
        if action == "clear":
            write_holdings([])
            return jsonify({"ok": True, "version": "v109", "holdings": [], "message": "로컬 표시목록 초기화. 다음 동기화 때 키움 실제잔고만 표시됩니다."})
        if action == "delete":
            # 실제잔고 기준 앱이므로 삭제는 임시 화면 삭제일 뿐입니다.
            code = v109_code(data.get("code"))
            items = [h for h in read_holdings() if v109_code(h.get("code")) != code]
            write_holdings(items)
            return jsonify({"ok": True, "version": "v109", "holdings": items, "message": "화면에서 임시 제거했습니다. 키움에 실제 보유 중이면 다시 동기화됩니다."})
    return jsonify(v109_force_sync_holdings(full_sync=True))
...
def api_v109_apply_screen_holdings():
    ...
    write_trade_state(state)
    return jsonify({"ok": True, "version": "v109", "holdings": items, "message": "스크린샷 기준 실제 보유종목을 임시 반영했습니다."})
...
def api_v109_stability_status():
    return jsonify({
        "ok": True, "version": "v109", "order_lock": True, "state_lock": True,
...
def api_v109_clear_price_cache():
    with PRICE_CACHE_LOCK:
        PRICE_CACHE.clear()
    return jsonify({"ok": True, "version": "v109", "message": "현재가 캐시 초기화 완료"})
...
def api_v109_trailing_test():
    ...
    return jsonify({"ok": True, "version": "v109", "test": h})
