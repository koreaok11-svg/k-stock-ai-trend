...
def check_one_holding(h):
    ...
    if target and cur>=target: send_holding_alert('target',h,cur)
    if stop and cur<=stop: send_holding_alert('stop',h,cur)
    return h
...
def watch_loop():
    ...
    while WATCH_STATE.get('running'):
        try:
            ...
            if time.time()-last_best>60: check_better_pick(); last_best=time.time()
...
def api_version(): return jsonify({'ok':True,'version':'clean-ui-v72','watch_interval':WATCH_INTERVAL})
